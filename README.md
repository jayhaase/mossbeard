...
CI lints YAML, builds the site, and checks internal links/images (`html-proofer`). Run the same steps locally:

```bash
./scripts/verify.sh   # runs bundle install automatically if gems are missing
```

Install [yamllint](https://yamllint.readthedocs.io/) if needed: `brew install yamllint` or `pip install yamllint`.
...
scripts/
  ...
  verify.sh              — yamllint + jekyll build + html-proofer (same as CI)
